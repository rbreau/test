...
function endSession() {
  ...
  const firstPassPerfect = q.items.slice(0, q.mode === 'quest' ? QUEST_LEN : ECHO_LEN).every(it => it.good);
  let lines = [];
  S.stats.sessions++;
  // streak
  const t = today();
  ...
  if (q.mode === 'quest') {
    const meta = q.items[0];
    const st = skillState(meta.skillId);
    const passed = q.correct >= Math.ceil(total * 0.85);
  ...
  save();
  const html = `
    <div class="m-eyebrow">${q.mode === 'echo' ? 'Echo Tide' : 'Trial Complete'}</div>
    <h3>${acc === 100 ? 'Perfection' : acc >= 85 ? 'A Strong Light' : acc >= 60 ? 'The Light Holds' : 'The Null Resists'}</h3>
    <div class="m-stats">
      <div>${q.correct}/${total}<span>solved</span></div>
      <div>+${q.xp}<span>xp</span></div>
      <div>+${q.lum + bonusLum}<span>lumins</span></div>
      <div>×${q.best}<span>best combo</span></div>
    </div>
...
function openShop() {
  const rows = SHOP.map(([id, g, name, desc, cost]) => {
    const owned = id === 'hints' ? `${S.items.hints} charges` : id === 'shield' ? `${S.items.shield} held` : `${S.items.boost} ready`;
    const maxed = id === 'shield' && S.items.shield >= 2;
