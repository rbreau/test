...
window.NumeraIsle = (function () {
  ...
  let grassU = { value: 0 }, pollen = null, fireflies = null, nightLights = [], anims = [], curIsle = null, curSkills = [];
  ...
  function applyDaylight(t) {
    ...
    if (bloomPass) { bloomPass.strength = 0.14 + 0.36 * L.night; bloomPass.threshold = 0.94 - 0.38 * L.night; bloomPass.radius = 0.45 + 0.15 * L.night; }
  ...
  function biome(isle, hue, n) {
    ...
    switch (isle.id) {
      case 'ember': { // lighthouse
        for (let i = 0; i < 4; i++) put(new THREE.CylinderGeometry(0.42 - i * 0.04, 0.46 - i * 0.04, 0.55, 20), std(i % 2 ? 0xe8503f : 0xf6f3ee), x, 0.275 + i * 0.55, z);
  ...
  function buildScene(isle, isleIndex, skills) {
    ...
    beacons = []; clouds = []; nightLights = []; anims = []; curIsle = isle; curSkills = skills;
    ...
    // town blocks (translucent teal), landmark, trees, grass, clouds
    const block = new THREE.MeshPhysicalMaterial({ color: 0x8fe6d6, transparent: true, opacity: 0.55, roughness: 0.25 });
    for (let i = 0; i < 16; i++) { const a = rnd() * Math.PI * 2, r = 6.9 + rnd() * 1.6, w = 0.5 + rnd() * 0.9, h = 0.3 + rnd() * 1.1, d = 0.5 + rnd() * 0.9; put(new THREE.BoxGeometry(w, h, d), block, Math.cos(a) * r, h / 2, Math.sin(a) * r).rotation.y = rnd() * Math.PI; }
    const avoid = biome(isle, hue, n);
    const leaf = std(hue.clone().lerp(new THREE.Color(0x2f9a5c), 0.75)), trunk = std(0x6b4b2e);
    for (let i = 0, tries = 0; i < 7 && tries < 40; tries++) {
      const a = rnd() * Math.PI * 2, r = 2.6 + rnd() * 1.7, x = Math.cos(a) * r, z = Math.sin(a) * r;
      if (Math.hypot(x - avoid.x, z - avoid.z) < avoid.r) continue;
      const h = 0.7 + rnd() * 0.4;
      put(new THREE.CylinderGeometry(0.05, 0.08, h, 8), trunk, x, h / 2, z);
      put(new THREE.SphereGeometry(0.3 + rnd() * 0.18, 14, 12), leaf, x, h + 0.22, z).scale.y = 1.2; i++;
    }
